...
military <- read_xlsx("data/gov_spending_per_capita.xlsx",  #reading in raw data, setting as "military"
                      sheet = "Share of Govt. spending",    #specifying the sheet we want to focus on
                      skip  = 7,                            #skips first 7 lines, since they are comments
                      n_max = 191,                          #max number of rows to read in (191)
                      na = c("xxx", ". ."))                  #specifies what values to read in as "NA"
```
...
> Filtering country rows that contains regions.  
...
military_long |>
  ...
  ggplot() +
  geom_boxplot(aes(y = fct_reorder(Country, -n), 
                   x = n)) +
  labs(x = "Proportion of Government Spending",
       y = "Eastern European Country")
